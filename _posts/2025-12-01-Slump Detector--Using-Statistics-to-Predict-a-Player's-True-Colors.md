...
2. Convert each odds thresholds to a `log` value so we can compare with the *log-likelihood ratio*.
...
5. Check if have a decision that we can make using SPRT. This is called our "check to stop".
...
### SPRT Example
Let's say we are the general manager of the Texas Rangers and we need to figure out whether Marcus Semien (their starting second baseman) should be traded because he is only a league-average hitter (0.200 batting average) or if we should keep him around because he is a star (0.275). We will say our acceptable probability for a 1) false alarm is 0.01 (1%) and 2) a missed opportunity is 0.1 (10%). Using the 2025 season, here is Semien's SPRT result for batting average:
